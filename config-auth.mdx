...
<Note>
Auth is only configurable in the [Enterprise version](/enterprise) of BuildBuddy.
</Note>

## OIDC

The Auth section enables BuildBuddy authentication using an OpenID Connect provider that you specify.

### Configuration Options

<ParamField path="oauth_providers" type="array">
  A list of configured OAuth Providers.
  
  <ParamField path="oauth_providers[].issuer_url" type="string" required>
    The issuer URL of this OIDC Provider.
  </ParamField>
  
  <ParamField path="oauth_providers[].client_id" type="string" required>
    The oauth client ID.
  </ParamField>
  
  <ParamField path="oauth_providers[].client_secret" type="string" required>
    The oauth client secret.
  </ParamField>
</ParamField>

<ParamField path="enable_anonymous_usage" type="boolean">
...
<Note>
Because [Gitlab has yet to support refresh tokens](https://gitlab.com/gitlab-org/gitlab/-/issues/16620), you need to configure BuildBuddy to not request the `offline_access` scope from Gitlab:
...
SAML 2.0 authentication is available for BuildBuddy Cloud (SaaS).
...
4. Enter the following fields under **General**, making sure to replace `<org-slug>` with the value from step 1:
...
6. In the newly created application view, navigate to `Single sign-on` and select `SAML`.
