...
<Warning>
To create API keys for users within an organization, this API must be called with an **Org admin key**. 

Org admin keys can be created via the BuildBuddy UI by navigating to **Settings > Org API keys**, then creating a new key with the "Org admin" capability.
</Warning>
...
<ParamField path="label" type="string">
  Optional display name for the API key. This is not required to be unique
  
  Example: `My API key`
</ParamField>

<ParamField path="expires_in" type="duration">
  Optional duration after which the API key should expire. If unset, the API key does not expire
  
  Example: `720h` (30 days), `8760h` (1 year)
</ParamField>

### Response

<ResponseField name="api_key" type="ApiKey">
  ...
  <Expandable title="ApiKey">
    ...
    <ResponseField name="value" type="string">
      ...
      <Warning>
      This is the only time the API key value will be returned. Store it securely.
      </Warning>
    </ResponseField>
    ...
    <ResponseField name="expiration_timestamp" type="timestamp">
      Timestamp at which the API key expires. If unset, the API key does not expire
    </ResponseField>
  </Expandable>
</ResponseField>
