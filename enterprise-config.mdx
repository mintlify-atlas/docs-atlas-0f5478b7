...
3. Put the username, password, IP address of your MySQL server, and database name into the BuildBuddy data_source connection string:
...
The configuration below demonstrates a default redis target:
...
Then, you can use the generated ca.crt and ca.pem files in your BuildBuddy configuration like this:
...
You'll also need to deploy executors to handle remote builds. The recommended way of deploying these is using our [Enterprise Helm Chart](/enterprise-helm).

For more information on configuring on-prem RBE, see our [enterprise on-prem RBE setup docs](/enterprise-rbe).
