...
description: Deploy BuildBuddy Remote Build Execution on-premises with Helm and Kubernetes
---
...
Then you'll need to make sure kubectl is configured with access to your Kubernetes cluster. Here are instructions for major cloud providers:

<CardGroup cols={3}>
  <Card title="Google Cloud" icon="google" href="https://cloud.google.com/kubernetes-engine/docs/how-to/cluster-access-for-kubectl">
    GKE cluster access setup
  </Card>
  <Card title="AWS" icon="aws" href="https://docs.aws.amazon.com/eks/latest/userguide/create-kubeconfig.html">
    EKS cluster access setup
  </Card>
  <Card title="Azure" icon="microsoft" href="https://docs.microsoft.com/en-us/azure/aks/kubernetes-walkthrough#connect-to-the-cluster">
...
<Check>
  You should now see a remote build execution checkbox and can try your first remote build execution. Make sure your RBE [toolchains and platforms](rbe-setup) are correctly configured.
</Check>
...
<Warning>
  If you scale beyond one app replica, make sure you're using MySQL instead of SQLite, and GCS/S3 instead of disk storage. For information on how to configure these, see the [enterprise configuration guide](enterprise-config). Scaling the number of executor replicas does not have these requirements.
</Warning>
...
- [buildbuddy-enterprise values.yaml file](https://github.com/buildbuddy-io/buildbuddy-helm/blob/master/charts/buildbuddy-enterprise/values.yaml)
- [buildbuddy-executor values.yaml file](https://github.com/buildbuddy-io/buildbuddy-helm/blob/master/charts/buildbuddy-executor/values.yaml)

<Tip>
  Values for the executor deployment are nested in the `executor:` block of the buildbuddy-enterprise yaml file.
</Tip>

## More configuration

For more configuration options beyond RBE, like authentication and storage options, see our:
- [Configuration docs](config)
- [Enterprise configuration guide](enterprise-config)
