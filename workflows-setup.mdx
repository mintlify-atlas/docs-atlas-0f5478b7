...
description: Set up BuildBuddy Workflows for continuous integration with your Bazel repositories hosted on GitHub
---

BuildBuddy Workflows is a Continuous Integration (CI) solution for Bazel repositories hosted on GitHub.

BuildBuddy Workflows are specifically designed for Bazel builds, and are tightly integrated with BuildBuddy RBE and Remote Caching, making them significantly faster than other CI options.

<Info>
See [Remote Runner Introduction](https://docs.buildbuddy.io/docs/remote-runner-introduction/) for more details on the benefits of Workflows.
</Info>
...
<Steps>
  <Step title="Log in and select organization">
    Log in to the BuildBuddy app, and use the organization picker in the sidebar to select your preferred organization.
  </Step>
  
  <Step title="Navigate to Workflows page">
    Navigate to the **Workflows** page using the sidebar.
  </Step>
  
  <Step title="Link a repository">
    Click **Link a repository**, then follow the steps displayed in the app.
  </Step>
</Steps>
...
<Tabs>
  <Tab title="Default Config">
    The default workflow config runs `bazel test //...` whenever a commit is pushed to your repo's default branch or a pull request branch is updated. 
    
    In order to enable this, click `Enable default workflow config` in the three-dot dropdown for your repository on the Workflows page.
  </Tab>
  
  <Tab title="Custom Config">
    To configure more complex workflows - i.e. different trigger conditions or running different commands - you will need to add a file called `buildbuddy.yaml` to the root of your git repo. 
    
    See [workflows configuration](workflows-config) for more details.
  </Tab>
</Tabs>
...
<Steps>
  <Step title="Navigate to GitHub settings">
    Go to **Settings** > **Branches** and find **Branch protection rules**.
  </Step>
  
  <Step title="Add or edit rule">
...
<Tip>
If you plan to use the Ubuntu 20.04 image (requires [advanced configuration](workflows-config#linux-image-configuration)), use `"gcr.io/flame-public/rbe-ubuntu20-04-workflows:latest"` in the command above.
</Tip>