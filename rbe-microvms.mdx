...
description: Run remote build actions in Firecracker microVMs with Docker support and preserved state across runs
---
...
environments, so the VMs do not become too large.
</Info>

For example, if you have some tests that use only MySQL, and others that only use PostgreSQL, they should have different `exec_properties`.
...
<Steps>
  <Step title="Build your custom image with Docker installed">
    Add Docker installation commands to your Dockerfile.
  </Step>
  
  <Step title="Test Docker installation">
  ...
  <Step title="Verify Docker is working">
    Once Docker is finished booting up, run the following command from another terminal. You should see "Hello world!" printed if Docker is properly installed:
